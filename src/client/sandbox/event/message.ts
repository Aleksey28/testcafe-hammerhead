...
export default class MessageSandbox extends SandboxBase {
    ...
    sendServiceMsg (msg, targetWindow: Window, ports?: Transferable[]) {
        ...
        const crossDomain     = isCrossDomainWindows(targetWindow, this.window);
        const canSendDirectly = !crossDomain && !!targetWindow[this.RECEIVE_MSG_FN];

        if (!canSendDirectly) {
            const origin          = nativeMethods.windowOriginGetter.call(this.window);
            const crossDomainPort = settings.get().crossDomainProxyPort;
            const targetOrigin    = crossDomain && crossDomainPort ? origin.replace(/\d*$/, crossDomainPort) : origin;
